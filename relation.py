...
async def create_user(session: AsyncSession, email, password, name):
...
async def get_all_user_positions(
    ...
    stmt = (
        ...
        .options(
            joinedload(Cart.product).joinedload(Product.car_brand),
            joinedload(Cart.product).joinedload(Product.car_series),
            joinedload(Cart.product).joinedload(Product.car_part),
        )
    )
    result: Result = await session.execute(statement=stmt)
